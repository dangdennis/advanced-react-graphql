...
const signedInMocksWithCartItems = [
  {
    request: { query: CURRENT_USER_QUERY },
    result: {
      data: { me: { ...fakeUser(), cart: [fakeCartItem(), fakeCartItem(), fakeCartItem()] } }
    }
  }
];
...
describe('<Nav/>', () => {
  it('renders a minimal nav when signed out', async () => {
    ...
    expect(toJSON(nav.children().length)).toMatchSnapshot();
  });
  ...
  it('renders the correct number of items in cart', async () => {
    const wrapper = mount(
